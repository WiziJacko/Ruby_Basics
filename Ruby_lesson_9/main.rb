...
class Main
  MAIN_MENU = <<~MAIN_QUESTIONS
    ...
    |                                                          |
    |   Введите "4", если хотите выбрать управление поездами   |
  ...
  TRAIN_ACTIONS_MENU = <<~TRAIN_ACTIONS
    ...
    |   Введите "2", если хотите заполнить вагон               |
    |   Введите "3", если хотите отцепить вагон                |
    |   Введите "4", если хотите увидеть все вагоны            |
    |   Введите "5", если хотите присвоить маршрут             |
    |   Введите "6", если хотите переместить поезд             |
    |                                                          |
    |   Введите "7", если хотите вернуться в главное меню      |
    ------------------------------------------------------------
  TRAIN_ACTIONS
  ...
  def train_actions(train)
    ...
    loop do
      ...
      when 2 then fill_carriage(train)
      when 3 then unhook_carriage(train)
      when 4 then show_all_carriages(train)
      when 5 then train_get_route(train)
      when 6 then train_move(train)
      when 7 then break
      else not_correct_input
      end
    end
